class CustomerPage {
  ...
  constructor() {
    this.state = {
      customerId: "",
      customer: null, //Will hold the returned customer object
      states: [] //List of US states from API
    };
    ...
    // Configures UI event bindings
    this.bindAllMethods();
    //Loads state list when page loads
    this.fetchStates();
    //Initializes UI in read-only mode
  ...
  fetchStates() {
    ...
    .then(data => { 
      if (data.length == 0) {
        ...
        this.state.states = data; //save states
        this.loadStates();
      }
    })
    .catch(error => {
